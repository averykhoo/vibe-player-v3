...
// Mock stores
let mockPlayerStoreValues: { speed: number; pitch: number; gain: number; [key: string]: any };
...
let mockAnalysisStoreValues: { vadPositiveThreshold: number; vadNegativeThreshold: number; [key: string]: any };
let mockAnalysisStoreWritable: Writable<typeof mockAnalysisStoreValues>;
vi.mock('$lib/stores/analysis.store', async () => {
    const { writable: actualWritable } = await import('svelte/store');
    mockAnalysisStoreValues = { vadPositiveThreshold: 0.5, vadNegativeThreshold: 0.35 }; // Default initial values
...
describe('Controls.svelte', () => {
    beforeEach(() => {
        ...
        act(() => {
            ...
            mockAnalysisStoreWritable.set({ vadPositiveThreshold: 0.5, vadNegativeThreshold: 0.35 });
        });
    });
    ...
    it('updates analysisStore when VAD Positive Threshold slider changes', async () => {
        ...
        render(Controls);
        const vadSlider = screen.getByLabelText<HTMLInputElement>(/VAD Positive Threshold/i);
    ...
    it('updates analysisStore when VAD Negative Threshold slider changes', async () => {
        const mockUpdate = vi.spyOn(mockAnalysisStoreWritable, 'update');
        render(Controls);
        const vadSlider = screen.getByLabelText<HTMLInputElement>(/VAD Negative Threshold/i);
    ...
    it('slider values update if store changes externally', async () => {
        ...
        act(() => {
             mockAnalysisStoreWritable.set({ vadPositiveThreshold: 0.9, vadNegativeThreshold: 0.1 });
        });
