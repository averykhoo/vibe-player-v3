...
import type { WorkerMessage, SileroVadInitPayload, SileroVadProcessResultPayload, SileroVadProcessPayload, SpectrogramInitPayload, SpectrogramResultPayload, SpectrogramProcessPayload } from '$lib/types/worker.types';
import { VAD_WORKER_MSG_TYPE, VAD_CONSTANTS, SPEC_WORKER_MSG_TYPE, VISUALIZER_CONSTANTS } from '$lib/utils'; // Assuming VAD_CONSTANTS is in utils/index
...
class AnalysisService {
    ...
    private pendingSpecRequests = new Map<string, { resolve: (value: any) => void, reject: (reason?: any) => void }>();
    ...
    public async initialize(options?: { positiveThreshold?: number, negativeThreshold?: number }): Promise<void> {
        ...
        analysisStore.update(s => ({ ...s, status: 'VAD service initializing...' }));
        ...
        this.worker.onmessage = (event: MessageEvent<WorkerMessage>) => {
            ...
            if (error) {
                ...
                serviceState.update(s => ({ ...s, error: `VAD Worker error: ${error}` }));
                 if (request) {
                    request.reject(error);
                    this.pendingRequests.delete(messageId!);
                }
                if (type === VAD_WORKER_MSG_TYPE.INIT_ERROR) {
                     serviceState.update(s => ({ ...s, isInitialized: false, isInitializing: false }));
                     analysisStore.update(s => ({ ...s, status: 'Error initializing VAD service.'}));
                }
                return;
            }
            ...
            switch (type) {
                case VAD_WORKER_MSG_TYPE.INIT_SUCCESS:
                    serviceState.update(s => ({ ...s, isInitialized: true, isInitializing: false }));
                    analysisStore.update(s => ({ ...s, status: 'VAD service initialized.' }));
                ...
                default:
                    console.log('AnalysisService received message from VAD worker:', event.data);
        ...
        this.worker.onerror = (err) => {
            ...
            serviceState.update(s => ({ ...s, error: `VAD Worker onerror: ${err.message}`, isInitialized: false, isInitializing: false }));
            analysisStore.update(s => ({ ...s, status: 'Critical VAD worker error.' }));
            this.pendingRequests.forEach(req => req.reject(new Error('VAD Worker failed critically.')));
        ...
        try {
            await this.postMessageToWorker({ type: VAD_WORKER_MSG_TYPE.INIT, payload: initPayload });
        } catch (err: any) {
            serviceState.update(s => ({ ...s, error: err.message || 'VAD Initialization failed', isInitialized: false, isInitializing: false }));
            analysisStore.update(s => ({ ...s, status: 'Error sending VAD init to worker.' }));
        }
    }

    public async analyzeAudioFrame(audioFrame: Float32Array, timestamp?: number): Promise<SileroVadProcessResultPayload | null> {
        ...
        try {
            const result = await this.postMessageToWorker({ type: VAD_WORKER_MSG_TYPE.PROCESS, payload });
            return result as SileroVadProcessResultPayload;
        } catch (error) {
            console.error("Error processing VAD frame:", error);
            analysisStore.update(s => ({...s, error: "Error processing VAD frame"}));
            return null;
        }
    }

    public async resetVadState(): Promise<void> {
        if (!get(serviceState).isInitialized || !this.worker)  throw new Error('VAD Service not initialized.');
        try {
            await this.postMessageToWorker({ type: VAD_WORKER_MSG_TYPE.RESET });
        } catch (error) {
            console.error("Error resetting VAD state:", error);
            analysisStore.update(s => ({...s, error: "Error resetting VAD state"}));
        }
    }

    // Placeholder for Goertzel DTMF detection logic
    // This would likely operate on raw audio frames or chunks
    public detectDTMF(audioFrame: Float32Array, sampleRate: number): string | null {
    ...
    public dispose(): void {
        ...
        analysisStore.update(s => ({ ...s, status: 'VAD service disposed.', isInitialized: false }));
    ...
    public async initializeSpectrogramWorker(options: { sampleRate: number, fftSize?: number, hopLength?: number }): Promise<void> {
        ...
        analysisStore.update(s => ({ ...s, spectrogramStatus: 'Spectrogram worker initializing...' }));

        this.spectrogramWorker.onmessage = (event: MessageEvent<WorkerMessage>) => {
            const { type, payload, error, messageId } = event.data;
            const request = messageId ? this.pendingSpecRequests.get(messageId) : undefined;

            if (error) {
                console.error(`Spectrogram Worker Error (type ${type}):`, error);
                analysisStore.update(s => ({ ...s, spectrogramError: `Worker error: ${error}` }));
                if (request) {
                    request.reject(error);
            ...
            switch (type) {
                case SPEC_WORKER_MSG_TYPE.INIT_SUCCESS:
                    ...
                    analysisStore.update(s => ({ ...s, spectrogramStatus: 'Spectrogram worker initialized.' }));
                    if (request) request.resolve(payload);
                    break;
        ...
        this.spectrogramWorker.onerror = (err) => { /* similar to vad worker onerror */
            console.error('Unhandled error in SpectrogramWorker:', err);
            analysisStore.update(s => ({ ...s, spectrogramError: `Worker onerror: ${err.message}`}));
            this.pendingSpecRequests.forEach(req => req.reject(new Error('Spectrogram Worker failed critically.')));
        ...
        const initPayload: SpectrogramInitPayload = {
            ...
            hopLength: options.hopLength || Math.floor((options.fftSize || VISUALIZER_CONSTANTS.SPEC_NORMAL_FFT_SIZE) / 4),
        };
        try {
            await this.postMessageToSpecWorker({ type: SPEC_WORKER_MSG_TYPE.INIT, payload: initPayload });
        } catch (e: any) {
            analysisStore.update(s => ({ ...s, spectrogramError: e.message || 'Spectrogram init failed'}));
            this.spectrogramInitialized.set(false);
        }
    }

    public async processAudioForSpectrogram(audioData: Float32Array): Promise<SpectrogramResultPayload | null> {
        if (!get(this.spectrogramInitialized)) throw new Error('Spectrogram worker not initialized.');
        ...
        try {
            ...
            const result = await this.postMessageToSpecWorker({ type: SPEC_WORKER_MSG_TYPE.PROCESS, payload });
    ...
    public disposeSpectrogramWorker(): void {
        if (this.spectrogramWorker) {
            ...
            this.spectrogramInitialized.set(false);
            this.pendingSpecRequests.clear();
            analysisStore.update(s => ({ ...s, spectrogramStatus: 'Spectrogram worker disposed.', spectrogramData: null }));
        }
    }

    public async startSpectrogramProcessing(audioBuffer: AudioBuffer): Promise<void> {
        if (!audioBuffer) {
            console.warn('AnalysisService: No audio buffer provided for spectrogram processing.');
            return;
        }

        if (!get(this.spectrogramInitialized)) {
            analysisStore.update(s => ({ ...s, spectrogramStatus: 'Initializing for new file...' }));
        ...
        if (!get(this.spectrogramInitialized)) {
             console.error('AnalysisService: Spectrogram worker failed to initialize for processing.');
             analysisStore.update(s => ({...s, spectrogramError: 'Worker init failed before processing.'}));
             return;
        }
        ...
        if (pcmData && pcmData.length > 0) {
            ...
            analysisStore.update(s => ({ ...s, spectrogramStatus: 'Spectrogram processing initiated.' }));
        } else {
            console.warn('AnalysisService: No PCM data to process for spectrogram.');
            analysisStore.update(s => ({ ...s, spectrogramStatus: 'No data for spectrogram.'}));
        }
    }
}
