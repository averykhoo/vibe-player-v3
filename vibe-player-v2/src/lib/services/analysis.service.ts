...
class AnalysisService {
  ...
  public async initialize(options?: AnalysisServiceInitializeOptions): Promise<void> {
    ...
    analysisStore.update(s => ({ ...s, vadStatus: "VAD service initializing...", vadInitialized: false, vadError: null }));
    ...
    this.worker.onmessage = (event: MessageEvent<WorkerMessage<unknown>>) => {
      ...
      if (error) {
        ...
        if (type === VAD_WORKER_MSG_TYPE.INIT_ERROR) {
            ...
            analysisStore.update(s => ({ ...s, vadStatus: "Error initializing VAD service.", vadInitialized: false }));
        }
      } else {
         switch (type) {
            case VAD_WORKER_MSG_TYPE.INIT_SUCCESS:
                ...
                this.isInitializing = false;
                analysisStore.update(s => ({ ...s, vadStatus: "VAD service initialized.", vadInitialized: true, vadError: null }));
            ...
            case VAD_WORKER_MSG_TYPE.PROCESS_RESULT:
                ...
                analysisStore.update(s => ({ ...s, lastVadResult: resultPayload, isSpeaking: resultPayload.isSpeech }));
                if (request) request.resolve(resultPayload);
                break;
            case `${VAD_WORKER_MSG_TYPE.RESET}_SUCCESS`:
                analysisStore.update(s => ({ ...s, vadStateResetted: true, lastVadResult: null, isSpeaking: false }));
                if (request) request.resolve(payload);
                break;
            default:
                if (request) request.resolve(payload);
    ...
    this.worker.onerror = (err: Event | string) => {
      ...
      analysisStore.update(s => ({ ...s, vadStatus: "Critical VAD worker error.", vadError: errorMsg, vadInitialized: false }));
      this.pendingRequests.forEach(req => req.reject(new Error(`VAD Worker failed critically: ${errorMsg}`)));
  ...
  public async analyzeAudioFrame(audioFrame: Float32Array, timestamp?: number): Promise<SileroVadProcessResultPayload | null> {
    ...
    try {
      ...
      analysisStore.update(s => ({ ...s, vadError: `Error processing VAD frame: ${errorMessage}` }));
      return null;
    }
  }
