...
vi.mock('$lib/stores/analysis.store', () => ({ // Though not directly used by audioEngine, good to have if future changes use it
    analysisStore: {
        subscribe: vi.fn(),
        set: vi.fn(),
        update: vi.fn()
...
describe('AudioEngineService', () => {
    ...
    describe('initialize', () => {
        it('should create a worker and post an INIT message', async () => {
            await audioEngineService.initialize({ sampleRate: 44100, channels: 1, initialSpeed: 1, initialPitch: 0 });
        ...
        it('should update playerStore on INIT_SUCCESS', async () => {
            const promise = audioEngineService.initialize({ sampleRate: 44100, channels: 1, initialSpeed: 1, initialPitch: 0 });
            expect(mockWorkerInstance.onmessage).not.toBeNull();

            // Simulate worker sending INIT_SUCCESS
            const mockEvent = { data: { type: RB_WORKER_MSG_TYPE.INIT_SUCCESS, messageId: 'rb_msg_0' } } as MessageEvent;
            mockWorkerInstance.onmessage!(mockEvent);

            await promise; // Ensure init promise resolves
            expect(playerStore.update).toHaveBeenCalledWith(expect.objectContaining({ status: 'Audio engine initialized.' }));
        });
        ...
        it('should update playerStore on INIT_ERROR', async () => {
            ...
            const mockErrorEvent = { data: { type: RB_WORKER_MSG_TYPE.INIT_ERROR, error: 'Test init error', messageId: 'rb_msg_0' } } as MessageEvent;
            mockWorkerInstance.onmessage!(mockErrorEvent);

            await promise; // Should resolve even on handled error (or reject if designed to)
            expect(playerStore.update).toHaveBeenCalledWith(expect.objectContaining({ status: 'Error initializing audio engine.' }));
        });
    });
    ...
    describe('loadFile', () => {
        ...
        beforeEach(async () => {
            ...
            const initPromise = audioEngineService.initialize({ sampleRate: 44100, channels: 1, initialSpeed: 1, initialPitch: 0 });
            if (mockWorkerInstance.onmessage) {
                 mockWorkerInstance.onmessage({ data: { type: RB_WORKER_MSG_TYPE.INIT_SUCCESS, messageId: 'rb_msg_0' } } as MessageEvent);
            }
            await initPromise;
            mockAudioContextInstance.decodeAudioData.mockResolvedValue(mockDecodedBuffer as unknown as AudioBuffer);
        });
        ...
        it('should call analysisService.startSpectrogramProcessing with the decoded buffer', async () => {
            await audioEngineService.loadFile(mockArrayBuffer, mockFileName);
            expect(analysisService.startSpectrogramProcessing).toHaveBeenCalledWith(mockDecodedBuffer);
        });
        ...
        it('should update playerStore on decoding error', async () => {
            ...
            await expect(audioEngineService.loadFile(mockArrayBuffer, mockFileName)).rejects.toThrow(decodeError.message);
            expect(playerStore.update).toHaveBeenCalledWith(
                expect.objectContaining({
    ...
    describe('play/pause/stop', () => {
        ...
        it('play should start playback and update store', () => {
            ...
            expect(playerStore.update).toHaveBeenCalledWith(expect.objectContaining({ isPlaying: true, status: 'Playing original audio...' }));
        });
        ...
        it('pause should stop playback (effectively) and update store', () => {
            ...
            expect(playerStore.update).toHaveBeenCalledWith(expect.objectContaining({ isPlaying: false, status: 'Playback paused.' }));
        });
        ...
        it('stop should stop playback, reset currentTime and update store', () => {
            ...
            expect(playerStore.update).toHaveBeenCalledWith(expect.objectContaining({ isPlaying: false, status: 'Playback stopped.', currentTime: 0 }));
        });
    });
    ...
    describe('dispose', () => {
        it('should terminate the worker and close AudioContext', async () => {
            ...
            expect(playerStore.update).toHaveBeenCalledWith(expect.objectContaining({ status: 'Audio engine disposed.', isInitialized: false }));
        });
    });
});
