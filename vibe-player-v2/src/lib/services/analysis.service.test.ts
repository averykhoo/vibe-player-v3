...
import { VAD_WORKER_MSG_TYPE, SPEC_WORKER_MSG_TYPE, VAD_CONSTANTS, VISUALIZER_CONSTANTS } from '$lib/utils';
...
describe('AnalysisService', () => {
    ...
    describe('initialize (VAD)', () => {
        ...
        it('should update analysisStore on VAD INIT_SUCCESS', async () => {
            ...
            await promise;
            expect(analysisStore.update).toHaveBeenCalledWith(expect.objectContaining({ status: 'VAD service initialized.' }));
        });

        it('should update analysisStore on VAD INIT_ERROR', async () => {
            const promise = analysisService.initialize();
            expect(mockVadWorkerInstance.onmessage).not.toBeNull();
            mockVadWorkerInstance.onmessage!({ data: { type: VAD_WORKER_MSG_TYPE.INIT_ERROR, error: 'VAD init fail', messageId: 'vad_msg_0' } } as MessageEvent);
            await promise;
            expect(analysisStore.update).toHaveBeenCalledWith(expect.objectContaining({ status: 'Error initializing VAD service.' }));
        });
    });
    ...
    describe('initializeSpectrogramWorker', () => {
        ...
        it('should update analysisStore on Spectrogram INIT_SUCCESS', async () => {
            const promise = analysisService.initializeSpectrogramWorker({ sampleRate: 16000 });
            expect(mockSpecWorkerInstance.onmessage).not.toBeNull();
            mockSpecWorkerInstance.onmessage!({ data: { type: SPEC_WORKER_MSG_TYPE.INIT_SUCCESS, messageId: 'spec_msg_0' } } as MessageEvent);
            await promise;
            expect(analysisStore.update).toHaveBeenCalledWith(expect.objectContaining({ spectrogramStatus: 'Spectrogram worker initialized.' }));
        });
    });
    ...
    describe('startSpectrogramProcessing', () => {
        beforeEach(async () => {
            ...
            if(mockVadWorkerInstance.onmessage) mockVadWorkerInstance.onmessage!({ data: { type: VAD_WORKER_MSG_TYPE.INIT_SUCCESS, messageId: 'vad_msg_0' } } as MessageEvent);
        ...
        it('should call initializeSpectrogramWorker if not already initialized', async () => {
            ...
            const processSpy = vi.spyOn(analysisService, 'processAudioForSpectrogram');
            ...
            initSpy.mockImplementation(async () => {
                 if(mockSpecWorkerInstance.onmessage) mockSpecWorkerInstance.onmessage!({ data: { type: SPEC_WORKER_MSG_TYPE.INIT_SUCCESS, messageId: 'spec_msg_0' } } as MessageEvent);
            ...
            expect(initSpy).toHaveBeenCalledWith({ sampleRate: mockAudioBuffer.sampleRate });
        ...
        it('should call processAudioForSpectrogram with PCM data', async () => {
            ...
            const initSpecPromise = analysisService.initializeSpectrogramWorker({ sampleRate: mockAudioBuffer.sampleRate });
            if(mockSpecWorkerInstance.onmessage) mockSpecWorkerInstance.onmessage!({ data: { type: SPEC_WORKER_MSG_TYPE.INIT_SUCCESS, messageId: 'spec_msg_0' } } as MessageEvent);
    ...
    describe('dispose', () => {
        it('should terminate both VAD and Spectrogram workers', async () => {
            ...
            expect(analysisStore.update).toHaveBeenCalledWith(expect.objectContaining({ status: 'VAD service disposed.' }));
            expect(analysisStore.update).toHaveBeenCalledWith(expect.objectContaining({ spectrogramStatus: 'Spectrogram worker disposed.' }));
        });
    });
});
