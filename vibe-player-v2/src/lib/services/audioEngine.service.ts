...
import type { WorkerMessage, RubberbandInitPayload, RubberbandProcessResultPayload } from '$lib/types/worker.types';
...
class AudioEngineService {
    ...
    public async unlockAudio(): Promise<void> {
        ...
        if (ctx.state === 'suspended') {
            try {
                ...
                playerStore.update(s => ({ ...s, error: 'Failed to resume audio context.' }));
            }
        }
    }
    ...
    public async initialize(options: { sampleRate: number, channels: number, initialSpeed: number, initialPitch: number, gain?: number }): Promise<void> {
        ...
        playerStore.update(s => ({ ...s, status: 'Audio engine initializing...' }));
        ...
        this.worker.onmessage = (event: MessageEvent<WorkerMessage>) => {
            ...
            if (error) {
                ...
                if (type === RB_WORKER_MSG_TYPE.INIT_ERROR) {
                     ...
                     playerStore.update(s => ({ ...s, status: 'Error initializing audio engine.'}));
                }
                return;
            }
            ...
            switch (type) {
                case RB_WORKER_MSG_TYPE.INIT_SUCCESS:
                    ...
                    playerStore.update(s => ({ ...s, status: 'Audio engine initialized.' }));
                ...
                case RB_WORKER_MSG_TYPE.PROCESS_RESULT:
                    ...
                    // For now, just log it
                    console.log('Received processed audio chunk:', resultPayload.outputBuffer);
                    playerStore.update(s => ({ ...s, lastProcessedChunk: resultPayload.outputBuffer }));
                ...
                default:
                    console.log('AudioEngineService received message from worker:', event.data);
        ...
        this.worker.onerror = (err) => {
            ...
            serviceState.update(s => ({ ...s, error: `Worker onerror: ${err.message}`, isInitialized: false, isInitializing: false }));
            playerStore.update(s => ({ ...s, status: 'Critical worker error.' }));
            // Reject all pending requests on a critical worker error
            this.pendingRequests.forEach(req => req.reject(new Error('Worker failed critically.')));
        ...
        try {
            await this.postMessageToWorker({ type: RB_WORKER_MSG_TYPE.INIT, payload: initPayload });
            ...
            serviceState.update(s => ({ ...s, error: err.message || 'Initialization failed', isInitialized: false, isInitializing: false }));
            playerStore.update(s => ({ ...s, status: 'Error sending init to worker.' }));
        }
    }
    ...
    public async loadFile(audioFileBuffer: ArrayBuffer, fileName: string): Promise<void> {
        ...
        if (!this.audioContextInternal) {
            playerStore.update(s => ({ ...s, error: 'AudioContext not available for decoding.' }));
            throw new Error('AudioContext not available for decoding.');
        }

        playerStore.update(s => ({ ...s, status: `Decoding ${fileName}...`, error: null, fileName }));
        try {
            this.originalAudioBuffer = await this.audioContextInternal.decodeAudioData(audioFileBuffer);
            ...
            console.log('Decoded audio and extracted waveform data:', this.originalAudioBuffer);
            ...
            //     });
            // }
             console.warn("AudioEngine not re-initialized with new file parameters automatically yet. Manual re-init might be needed if SR/channels change.");

        } catch (e: any) {
            console.error('Error decoding audio data:', e);
            playerStore.update(s => ({ ...s, status: `Error decoding ${fileName}.`, error: e.message, isPlayable: false, waveformData: undefined })); // Clear waveform data on error
            throw e;
        }
    }
    ...
    public setGain(level: number): void {
        if (this.gainNode) {
            this.gainNode.gain.setValueAtTime(level, this.audioContextInternal?.currentTime || 0);
    ...
    public play(): void {
        ...
        if (this.originalAudioBuffer && this.audioContextInternal && this.gainNode) {
            ...
            this.decodedAudioPlayerNode = this.audioContextInternal.createBufferSource();
            ...
            this.decodedAudioPlayerNode.onended = () => {
                ...
                playerStore.update(s => ({ ...s, status: 'Playback ended.', isPlaying: false }));
            ...
            playerStore.update(s => ({ ...s, status: 'Playing original audio...', isPlaying: true }));
            console.log('AudioEngine: Play called. Playing original buffer.');
        } else {
            playerStore.update(s => ({ ...s, status: 'No audio loaded or engine not ready.', error: 'Cannot play: No audio loaded.' }));
            console.log('AudioEngine: Play called, but no original audio buffer or audio context.');
        }
    }
    ...
    public pause(): void {
        ...
        if (this.decodedAudioPlayerNode && this.audioContextInternal) {
            ...
            const currentTime = this.audioContextInternal.currentTime - (this.decodedAudioPlayerNode as any)._startTime || 0;

            playerStore.update(s => ({ ...s, currentTime: currentTime >=0 ? currentTime: 0 }));
            this.decodedAudioPlayerNode.stop();
            // this.decodedAudioPlayerNode = null; // Don't nullify here if we want to "resume" by creating a new one
            this.isPlaying.set(false);
            playerStore.update(s => ({ ...s, status: 'Playback paused.', isPlaying: false }));
    ...
    public stop(): void {
        ...
        this.isPlaying.set(false);
        playerStore.update(s => ({ ...s, status: 'Playback stopped.', isPlaying: false, currentTime: 0 }));
        console.log('AudioEngine: Stop called.');
    }

    public async setSpeed(speed: number): Promise<void> {
        if (!get(serviceState).isInitialized || !this.worker) throw new Error('Service not initialized.');
        await this.postMessageToWorker({ type: RB_WORKER_MSG_TYPE.SET_SPEED, payload: { speed } });
        playerStore.update(s => ({ ...s, speed }));
    }

    public async setPitch(pitch: number): Promise<void> {
        if (!get(serviceState).isInitialized || !this.worker) throw new Error('Service not initialized.');
        await this.postMessageToWorker({ type: RB_WORKER_MSG_TYPE.SET_PITCH, payload: { pitch } });
        playerStore.update(s => ({ ...s, pitch }));
    }

    public async processAudioChunk(inputBuffer: Float32Array[]): Promise<RubberbandProcessResultPayload | null> {
        if (!get(serviceState).isInitialized || !this.worker) throw new Error('Service not initialized.');
        try {
            const result = await this.postMessageToWorker({ type: RB_WORKER_MSG_TYPE.PROCESS, payload: { inputBuffer } });
    ...
    public async flush(): Promise<RubberbandProcessResultPayload | null> {
        if (!get(serviceState).isInitialized || !this.worker) throw new Error('Service not initialized.');
        try {
            const result = await this.postMessageToWorker({ type: RB_WORKER_MSG_TYPE.FLUSH });
            return result as RubberbandProcessResultPayload;
        } catch (error) {
    ...
    public playAudioBuffer(buffer: Float32Array[], sampleRate: number) {
        ...
        const audioBuffer = ctx.createBuffer(buffer.length, buffer[0].length, sampleRate);
    ...
    public dispose(): void {
        ...
        playerStore.update(s => ({ ...s, status: 'Audio engine disposed.', isInitialized: false }));
