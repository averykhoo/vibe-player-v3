...
class AudioEngineService {
	...
	public async loadFile(audioFileBuffer: ArrayBuffer, fileName: string): Promise<void> {
		...
		try {
			...
			playerStore.update((s) => ({
				...
				sampleRate: this.originalBuffer!.sampleRate // ADDED
			}));
			console.log(
		...
		if (this.nextChunkTime < now + lookahead) {
			...
			if (this.sourcePlaybackOffset < this.originalBuffer.duration) {
				...
				const processPayload: RubberbandProcessPayload = {
					inputBuffer: [segment], // Note: The worker expects an array of Float32Arrays
					isFinalChunk:
						this.sourcePlaybackOffset + actualChunkDuration >= this.originalBuffer.duration
				};
	...
	/**
	 * [LOGGING ADDED] Sets the playback speed.
	 * @param {number} speed - The desired playback speed (e.g., 1.0 for normal).
	 */
	public setSpeed(speed: number): void {
		// LOG: Log the received value before processing.
	...
	/**
	 * [LOGGING ADDED] Sets the playback pitch.
	 * @param {number} pitch - The desired pitch shift in semitones (e.g., 0.0 for normal).
	 */
	...
	/**
	 * [LOGGING ADDED] Sets the master gain (volume) level.
	...
	public setGain(level: number): void {
		...
		if (this.gainNode && this.audioContext) {
			// Note: The store expects a value probably from 0-2, but WebAudio gain is linear.
			// The component might be doing a mapping, but the service takes the raw value.
			const newGain = Math.max(0, Math.min(2, level)); // Clamping based on UI slider range
	...
	private scheduleChunkPlayback(
		...
		if (!this.audioContext || !this.gainNode || this.isStopping || !this.originalBuffer) {
		...
		if (processedChannels.length !== this.originalBuffer!.numberOfChannels) {
			console.error(
				`ScheduleChunkPlayback: Processed chunk channel count (${processedChannels.length}) does not match original buffer (${this.originalBuffer!.numberOfChannels}).`
			);
			return;
		}
		// --- END CONSOLE.ERROR ---
		...
		if (frameCount === 0) {
			// console.log("ScheduleChunkPlayback: Received chunk with zero frames, skipping.");
			return;
		}
