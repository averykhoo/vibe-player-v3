...
// Existing imports:
import type { WorkerMessage, SpectrogramInitPayload, SpectrogramProcessPayload, SpectrogramResultPayload } from '../types/worker.types';
    ...
    try {
        switch (type) {
            case SPEC_WORKER_MSG_TYPE.INIT:
                ...
                self.postMessage({ type: SPEC_WORKER_MSG_TYPE.INIT_SUCCESS, messageId });
                break;
            ...
            case SPEC_WORKER_MSG_TYPE.PROCESS:
                ...
                for (let i = 0; (i + fftSize) <= audioData.length; i += hopLength) {
                 ...
                 if (magnitudes.length > 0) {
                    ...
                    self.postMessage({ type: SPEC_WORKER_MSG_TYPE.PROCESS_RESULT, payload: resultPayload, messageId });
                } else {
                     self.postMessage({ type: SPEC_WORKER_MSG_TYPE.PROCESS_RESULT, payload: {magnitudes: []}, messageId }); // Send empty if no frames
                }
                break;
            default:
