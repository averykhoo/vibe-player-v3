...
import type { WorkerMessage, RubberbandInitPayload, RubberbandProcessPayload, RubberbandProcessResultPayload } from '../types/worker.types';
        ...
        switch (type) {
            case RB_WORKER_MSG_TYPE.INIT:
                ...
                rubberbandInstance = new RubberBand(
                    ...
                    RubberBand.OptionProcessRealTime | RubberBand.OptionTransientsSmooth | RubberBand.OptionDetectorCompound,
            ...
            case RB_WORKER_MSG_TYPE.PROCESS:
                ...
                const processedFrames = rubberbandInstance.process(processPayload.inputBuffer[0], false); // false = not final chunk
                ...
                if (available > 0) {
                    ...
                    const retrieved = rubberbandInstance.retrieve(outputBuffer[0], available); // Assuming outputBuffer[0] is where data is put for mono
                    ...
                    const resultPayload: RubberbandProcessResultPayload = { outputBuffer: [ new Float32Array(outputBuffer[0].buffer, 0, retrieved) ] };
                     self.postMessage({ type: RB_WORKER_MSG_TYPE.PROCESS_RESULT, payload: resultPayload, messageId });
                }
                break;
            ...
            case RB_WORKER_MSG_TYPE.FLUSH:
                ...
                if (finalAvailable > 0) {
                     ...
                     const finalRetrievedSamples = rubberbandInstance.retrieve(new Float32Array(finalAvailable));
                     const flushResult: RubberbandProcessResultPayload = { outputBuffer: [finalRetrievedSamples] };
                     self.postMessage({ type: RB_WORKER_MSG_TYPE.FLUSH_RESULT, payload: flushResult, messageId });
                } else {
                     self.postMessage({ type: RB_WORKER_MSG_TYPE.FLUSH_RESULT, payload: { outputBuffer: [] }, messageId });
                }
