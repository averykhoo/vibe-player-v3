...
class GoertzelFilter {
  ...
  constructor(public targetFrequency: number, public sampleRate: number, N: number) {
    this.N = N;
...
class DTMFParser {
  ...
  constructor(private sampleRate: number, private blockSize: number) {
    this.lowGroupFilters = DTMF_FREQUENCIES_LOW.map(freq => new GoertzelFilter(freq, this.sampleRate, this.blockSize));
    this.highGroupFilters = DTMF_FREQUENCIES_HIGH.map(freq => new GoertzelFilter(freq, this.sampleRate, this.blockSize));
  }
  ...
  public processAudioBlock(audioBlock: Float32Array): string | null {
    ...
    if (maxLowMag < DTMF_ABSOLUTE_MAGNITUDE_THRESHOLD || maxHighMag < DTMF_ABSOLUTE_MAGNITUDE_THRESHOLD) {
      return null;
    }

    // Check relative threshold
    for (const freq in lowMagnitudes) {
        if (Number(freq) !== detectedLowFreq && lowMagnitudes[freq] * DTMF_RELATIVE_THRESHOLD_FACTOR > maxLowMag) return null;
    }
    for (const freq in highMagnitudes) {
        if (Number(freq) !== detectedHighFreq && highMagnitudes[freq] * DTMF_RELATIVE_THRESHOLD_FACTOR > maxHighMag) return null;
    }
