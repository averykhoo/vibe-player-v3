...
export class PlayerPage {
  ...
  async expectControlsToBeReadyForPlayback() {
    // Wait for the status display to explicitly say "Ready", which indicates
    // the worker handshake is complete and `isPlayable` is true.
    await expect(this.fileStatusDisplay).toHaveText(/Ready/, {
      timeout: 20000,
    });

    // Existing checks can remain as secondary confirmation.
    await expect(
      this.timeDisplay,
