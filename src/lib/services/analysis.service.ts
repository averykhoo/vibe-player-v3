...
import {SPEC_WORKER_MSG_TYPE, VAD_WORKER_MSG_TYPE} from "$lib/types/worker.types";
...
class AnalysisService {
  ...
  private isSpectrogramInitialized = false;
    ...
    public async initialize(options?: AnalysisServiceInitializeOptions): Promise<void> {
        console.log('[AnalysisService] Initialize called.');
        ...
        this.worker.onmessage = (event: MessageEvent<WorkerMessage<unknown>>) => {
            ...
            const request = messageId
                ...
                : undefined;
            console.log(`[AnalysisService] Received message from VAD worker: type=${type}`);
            ...
            switch (type) {
                case VAD_WORKER_MSG_TYPE.INIT_SUCCESS:
                    console.log('[AnalysisService] VAD Worker reported INIT_SUCCESS.');
        ...
        try {
            console.log('[AnalysisService] Posting INIT message to VAD worker.');
    ...
    public async processAudioForSpectrogram(
        ...
        if (!this.spectrogramWorker || !this.isSpectrogramInitialized) {
        ...
        try {
            ...
            analysisStore.update((s: AnalysisState) => ({
                ...s,
                spectrogramError: `Error processing for spectrogram: ${errorMessage}`
            }));
            return null;
        }
    }
