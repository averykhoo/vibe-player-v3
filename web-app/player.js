...
const AudioPlayer = (function () {
    ...
    let fileInput, fileInfo, playPauseButton, jumpBackButton, jumpForwardButton,
        ...
        vadNegativeThresholdSlider, vadNegativeThresholdValueDisplay; // Added Negative
    ...
    // Store CURRENT thresholds used for recalculation
    let currentVadPositiveThreshold = 0.5;
    let currentVadNegativeThreshold = 0.35;
    ...
    function assignDOMElements() {
        ...
        vadNegativeThresholdSlider = document.getElementById('vadNegativeThreshold'); // Added
        vadNegativeThresholdValueDisplay = document.getElementById('vadNegativeThresholdValue'); // Added
    }
    ...
    function setupEventListeners() {
        ...
        // --- Setup VAD Tuning Listeners ---
        if (vadThresholdSlider) {
            vadThresholdSlider.addEventListener('input', handleThresholdChange); // Use same handler
        }
        if (vadNegativeThresholdSlider) { // Added
    ...
    async function handleFileLoad(e) {
        ...
        vadFrameSamples = 0;
        vadSampleRate = 0;
        // Reset current thresholds to defaults (or desired initial values)
        currentVadPositiveThreshold = 0.5;
        currentVadNegativeThreshold = 0.35;
        ...
        try {
            ...
            // --- Perform VAD Analysis (get initial regions and probabilities) ---
            console.log("Preparing audio for VAD...");
            ...
            // Define initial options (could be constants or derived)
            const initialVadOptions = {
                positiveSpeechThreshold: currentVadPositiveThreshold, // Use default
                negativeSpeechThreshold: currentVadNegativeThreshold, // Use default
                // redemptionFrames: 7 // Or pass from options if needed
            };
            const vadResult = await SileroVAD.analyzeAudio(pcm16kMono, 16000, initialVadOptions);
            console.timeEnd("VAD Analysis");

            // --- Store VAD results and parameters ---
            speechRegions = vadResult.regions; // Store initial regions calculated by analyzeAudio
            console.log("Initial speechRegions from VAD:", JSON.stringify(speechRegions)); // Log for debugging
            ...
            // Update current thresholds based on what was ACTUALLY used (in case defaults changed in silero_vad.js)
            currentVadPositiveThreshold = vadResult.initialPositiveThreshold;
            currentVadNegativeThreshold = vadResult.initialNegativeThreshold;

            // --- Set Slider Initial Values and Enable ---
            if (vadThresholdSlider) {
                vadThresholdSlider.value = currentVadPositiveThreshold;
                vadThresholdSlider.disabled = false; // Enable slider
            }
            if (vadThresholdValueDisplay) {
                vadThresholdValueDisplay.textContent = currentVadPositiveThreshold.toFixed(2);
            }
            if (vadNegativeThresholdSlider) { // Added
                vadNegativeThresholdSlider.value = currentVadNegativeThreshold;
                vadNegativeThresholdSlider.disabled = false; // Enable slider
            }
            if (vadNegativeThresholdValueDisplay) { // Added
                vadNegativeThresholdValueDisplay.textContent = currentVadNegativeThreshold.toFixed(2);
            }
            // --- End Set Sliders ---

            updateSpeechRegionsDisplay(); // Update text display with initial regions

            // Compute and draw visuals using the ORIGINAL decodedBuffer and INITIAL speechRegions
            computeAndDrawVisuals(); // This will use the initial `speechRegions`

            // --- Force Recalculation & Redraw using Slider Logic ---
            // This ensures the waveform immediately reflects the state represented by the sliders,
            // using the exact same logic that works when the user moves the slider.
            // We simulate an 'input' event on one of the sliders.
            console.log("Forcing initial redraw based on slider state.");
            if (vadThresholdSlider) {
                // Create a mock event object targeting the slider
                const mockEvent = {target: vadThresholdSlider};
                handleThresholdChange(mockEvent); // Call the handler
            }
            // --- End Force Redraw ---

        } catch (err) {
            // ... (error handling) ...
            disableControls(); // Ensure sliders are disabled on error
            // ... (revoke object URL) ...
        } finally {
    ...
    function resetUI() {
        ...
        disableControls(); // Disable all controls, including VAD sliders
    }
    ...
    function disableControls() {
        ...
        // Disable VAD sliders
        if (vadThresholdSlider) vadThresholdSlider.disabled = true;
        if (vadNegativeThresholdSlider) vadNegativeThresholdSlider.disabled = true;
    }
    ...
    /**
     * Event Handler for BOTH VAD Threshold Slider changes.
     * Updates the corresponding current threshold, recalculates regions, and redraws.
     * @param {Event} e - The input event from the slider.
     */
    function handleThresholdChange(e) {
        if (!vadProbabilities || !decodedBuffer) return; // No data to process

        const slider = e.target;
        const newValue = parseFloat(slider.value);

        // Update the correct current threshold and display based on which slider changed
        if (slider.id === 'vadThreshold') {
            currentVadPositiveThreshold = newValue;
            if (vadThresholdValueDisplay) {
                vadThresholdValueDisplay.textContent = newValue.toFixed(2);
            }
            // Optional: Auto-adjust negative threshold relative to positive?
            // currentVadNegativeThreshold = Math.max(0.01, Math.min(newValue - 0.15, 0.99));
            // if (vadNegativeThresholdSlider) vadNegativeThresholdSlider.value = currentVadNegativeThreshold;
            // if (vadNegativeThresholdValueDisplay) vadNegativeThresholdValueDisplay.textContent = currentVadNegativeThreshold.toFixed(2);
        } else if (slider.id === 'vadNegativeThreshold') {
            currentVadNegativeThreshold = newValue;
            if (vadNegativeThresholdValueDisplay) {
                vadNegativeThresholdValueDisplay.textContent = newValue.toFixed(2);
            }
        } else {
        ...
        // Define options using the CURRENT threshold values
        const options = {
            positiveSpeechThreshold: currentVadPositiveThreshold,
            negativeSpeechThreshold: currentVadNegativeThreshold,
        ...
        // Redraw *only* the waveform with the new regions
        // (Recomputing waveform data might not be strictly necessary if canvas size hasn't changed)
        const waveformData = computeWaveformData(decodedBuffer, waveformCanvas.width);
        drawWaveform(waveformData, waveformCanvas, speechRegions, decodedBuffer.duration);

        // updateUI(); // Optional: Update progress indicator (unlikely needed here)
        // --- End Recalculate and Redraw ---
    }
