...
export class AudioOrchestrator {
    ...
    private async _processAudioBuffer(buffer: ArrayBuffer, name: string): Promise<void> {
        statusStore.set({ message: `Decoding audio: ${name}...`, type: 'info', isLoading: true });
        let audioBuffer: AudioBuffer;
        try {
            audioBuffer = await audioEngine.decodeAudioData(buffer);
        } catch (e: any) {
            console.error("Error decoding audio data:", e);
            this.handleError(new Error(`Failed to decode audio: ${e.message || 'Unknown decoding error'}`));
            throw e; // Re-throw to be caught by loadFileAndAnalyze
        }

        statusStore.set({ message: `Initializing audio engine for ${name}...`, type: 'info', isLoading: true });
        try {
            await audioEngine.initializeWorker(audioBuffer);
        } catch (e: any) {
            console.error("Error initializing worker:", e);
            this.handleError(new Error(`Failed to initialize audio engine: ${e.message || 'Unknown worker init error'}`));
            throw e; // Re-throw to be caught by loadFileAndAnalyze
        }
        ...
        statusStore.set({ isLoading: false, message: `Ready: ${name}`, type: 'success' });
        ...
        Promise.allSettled([
            ...
            results.forEach((result, index) => {
                if (result.status === 'rejected') {
                    const serviceName = index === 0 ? 'DTMF analysis' : 'Spectrogram analysis';
                    console.error(`${serviceName} for ${fileName} failed:`, result.reason);
    ...
    public async loadFileAndAnalyze(file: File): Promise<void> {
        if (this.isBusy) {
            ...
            statusStore.set({ message: `Player is busy. Please wait.`, type: 'warning', isLoading: true });
            return;
        }
        this.isBusy = true;
        ...
        analysisStore.update(s => ({ ...s, dtmfResults: [], spectrogramData: null })); // Clear analysis
        ...
        try {
            ...
            } else if (!get(playerStore).isPlayable && currentStatus.type !== 'error') {
    ...
    public updateUrlFromState = (): void => {
        ...
        if (pStore.fileName && pStore.isPlayable) { // Only add file if playable
    ...
    public setupUrlSerialization(): void {
        ...
        playerStore.subscribe(({ speed, pitchShift, gain, fileName, isPlayable, duration }) => {
            ...
            if (isPlayable) { // Only update URL if a track is considered playable
                debouncedUpdater();
            } else {
